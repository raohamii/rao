...
class HrLoan(models.Model):
    ...
    def action_approve(self):
        ...
        if loan_approve:
            ...
            for loan in self:
                ...
                loan_des=''
                if loan.description:
                    loan_des+='Loan For' + ' ' + loan.employee_id.name+' '+str(loan.description)
                else:
                    loan_des+='Loan For' + ' ' + loan.employee_id.name
                vals = {
                    'name': loan_des,
                    ...
                    'journal_id': loan.journal_id.id,
                    # 'date': date.today(),
                    'date': loan.loan_disburse_date,
                    'line_ids': [(0, 0, debit_vals), (0, 0, credit_vals)]
                }
                move = self.env['account.move'].create(vals)
                # move.date = loan.loan_disburse_date
                move.action_post()
    ...
    def action_double_approve(self):
        ...
        for loan in self:
            ...
            if loan.description:
                desc='Loan For' + ' ' + loan.employee_id.name+' '+ str(loan.description)
            else:
               desc= 'Loan For' + ' ' + loan.employee_id.name + ' '
            vals = {
                'name': desc,
                'narration': loan.employee_id.name,
                'ref': loan.name,
