...
class HrPayslip(models.Model):
                    ...
                    if contract_start_date <= current_day:  # Only validate if the contract started before or on the current day
                        ...
                        if not public_holiday and not employee_on_leave:
                            ...
                            if 'params' in self.env.context:
                                if not "hr.leave" in self.env.context['params']['model']:
    ...
    # def get_eobi(self, payslip):
    #     amount = payslip.contract_id.minimum_eobi
    #     amount_total=0
    #     if payslip.contract_id.minimum_eobi != 0:
    #         now = datetime.now()
    #         if payslip.date_from.month == payslip.contract_id.date_start.month:
    #             work_entries = self.env['hr.work.entry']._read_group(
    #                 payslip.contract_id._get_work_hours_domain(payslip.date_from, payslip.date_to, domain=None, inside=True),
    #                 ['work_entry_type_id'],
    #                 ['duration:sum']
    #             )
    ...
    #             # total_hours = work_data[1]
    #             total_hours = 0
    #             if len(work_entries) > 0:
    #                 total_hours = work_entries[0][1]
    #             total_attended_days = (total_hours/8)
    #             per_day = (payslip.contract_id.wage/26)
    #             calculated_pay = total_attended_days * per_day
    #             work_Days_line_ids_amount=0
    #             for all_records in payslip.worked_days_line_ids:
    #                 work_Days_line_ids_amount+=all_records.amount
    #             if work_Days_line_ids_amount < 32000:
    #                 amount_total = work_Days_line_ids_amount * 0.01
    #             else:
    #                 amount_total=payslip.contract_id.minimum_eobi
